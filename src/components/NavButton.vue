<template>
  <div
    class="fixed w-screen h-screen bg-white z-30 transition-transform duration-500 ease-in-out -translate-y-full"
    :class="{ 'translate-y-0': linkClicked }"
  ></div>
  <div
    class="nav-bar fixed w-screen h-screen bg-almost-black z-10"
    :class="{ opened }"
  ></div>
  <div
    v-if="showMenu"
  ...
  >
    <h2
      class="w-max absolute left-24 top-1/2 -translate-y-1/2 text-humongous font-bold opacity-5 tracking-wider uppercase pointer-events-none"
    >
      Lee Jia Jie
    </h2>
    <span
      @click="clickLink('/')"
      class="text-white text-5xl font-bold cursor-pointer transition-opacity opacity-40 hover:opacity-100"
      >Home</span
    >
    <span
