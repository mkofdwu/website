<template>
  <div class="h-screen relative">
    <div class="flex flex-col items-center">
      <div class="relative w-1/2 overflow-clip mt-16 mb-14">
        <div class="flex items-center transition-transform duration-500 cursor-zoom-in"
          :style="`transform: translateX(-${carouselIndex}00%)`">
          <img v-for="image in project.images" :key="image" :src="image" class="w-50s h-1/2 object-cover"
        ...
        <button v-if="carouselIndex < project.images.length - 1"
        ...
        <div v-if="project.images.length > 1" class="flex gap-x-4 absolute left-1/2 -translate-x-1/2 bottom-5">
          <div v-for="i in project.images.length" :key="i"
            class="w-2 h-2 rounded-full bg-white transition-opacity cursor-pointer"
            :style="`opacity: ${carouselIndex === i - 1 ? 1 : 0.4}`" @click="carouselIndex = i - 1"></div>
        </div>
      </div>
