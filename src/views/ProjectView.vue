<template>
  <div class="h-screen relative">
    <div class="flex flex-col items-center">
      <img :src="project.images[0]" class="w-1/2 h-1/2 mt-16 mb-14 object-cover" />
      <div class="w-1/2 px-10 flex flex-col items-start">
    ...
    <router-link to="/projects"
      class="absolute top-24 left-24 flex uppercase tracking-wider hover:tracking-hover active:tracking-normal active:opacity-60 transition-all">
      <span class="material-symbols-outlined mr-3">chevron_left</span>
      Back to projects
    </router-link>
    <div class="absolute top-1/2 -translate-y-1/2 right-24 flex flex-col items-center gap-y-8">
      <router-link v-for="project in projects" :key="project.id" :to="`/project/${project.id}`"
        :disabled="project.id === thisId" class="bg-almost-black rounded-full transition-all"
        :class="project.id === thisId ? 'w-3 h-3' : 'w-2 h-2 opacity-20'"></router-link>
    </div>
  </div>
</template>
